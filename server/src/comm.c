...
void send_payload(server_payload_t *payload, int destination) {
  ...
  MPI_Send(&(*payload), SERVER_PAYLOAD_SIZE, MPI_BYTE, destination, 0,
           MPI_COMM_WORLD);
}
...
void recv_result(worker_result_t *result, int source) {
  ...
  MPI_Recv(&encrypted_result, WORKER_RESULT_SIZE, MPI_BYTE, source, 0,
           MPI_COMM_WORLD, MPI_STATUS_IGNORE);
...
void send_result(payload_worker_result_t *result) {
  ...
  // TODO: Cambiar para user todo el payload, el ->data[0] es por mientras
  memcpy(data, &(result->data[0]), WORKER_RESULT_SIZE);
